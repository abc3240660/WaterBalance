...
void    FPPA0 (void)
{
    ...
    // IN Pull-UP
    PAPH        =        _FIELD(p_InA_M, p_InA_V, p_InA_H);
    ...
    BYTE    Key_Flag;
    Key_Flag            =    _FIELD(p_InA_M, p_InA_V, p_InA_H);
...
//    fppen    =    0xFF;
    ...
    while (1) {
        ...
        while (t16_10ms) {
			...
			if (0 == start) {
				if (cnt_3s_time_startup < 111) {
					cnt_3s_time_startup++;
				}

				if (10 == cnt_3s_time_startup) {
				...
				} else if (60 == cnt_3s_time_startup) {
					if (2 == stepx) {
						if (f_IN_QV2) {
							#ifndef GREEN_PWM
								p_OutB_V2 = 1;

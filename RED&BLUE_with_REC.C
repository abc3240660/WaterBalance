		...
		if (INTRQ.T16) {// TM16 Interrupt 20KHz (=50us)
            ...
            if (!p_InB_X) {// LOW
                if (f_sync_trig) {
                    ...
                    if ((f_InB_X_last) && (h_cnt_dat != 0)) {// Last H
                        if ((h_cnt_dat>=6) && (h_cnt_dat<=8)) {// 0.256ms ~ 0.512ms H
							data_sta = 0;
                            dat_bit_cnt++;
                        } else if ((h_cnt_dat>=36) && (h_cnt_dat<=40)) {// 0.896ms ~ 1.024ms H
							data_sta = 1;
                            dat_bit_cnt++;
                        } else {
							dat_bit_cnt = 0;
                    ...
                    if (1 == sync_sta) {
                        l_cnt_sync++;
                        if ((l_cnt_sync>=190) && (l_cnt_sync<=200)) {// 9.856ms ~ 10.112ms L
                            sync_sta = 2;// sync stage2 ok
                        } else if (l_cnt_sync > 200) {
                ...
                if (f_sync_trig) {
                    ...
                    if ((!f_InB_X_last) && (l_cnt_dat != 0)) {// Last L
                        if ((l_cnt_dat>=6) && (l_cnt_dat<=8)) {// 0.256ms ~ 0.512ms H
							data_sta = 1;
                            dat_bit_cnt++;
                        } else if ((l_cnt_dat>=36) && (l_cnt_dat<=40)) {// 0.896ms ~ 1.024ms H
							data_sta = 0;
							dat_bit_cnt++;
                        } else {
							dat_bit_cnt = 0;
                    ...
                    if ((h_cnt_sync>=6) && (h_cnt_sync<=8)) {// 0.256ms ~ 0.512ms H

...
void FPPA0(void)
{
    ...
    BIT     f_In12_last     :    Sys_FlagX.6;
    BIT     f_sync_trig     :    Sys_FlagX.7;
        ...
        if (INTRQ.T16) {
            ...
            if (!p_In12) {// LOW
                if (f_sync_trig) {
                    ...
                    if ((f_In12_last) && (h_cnt_dat != 0)) {// Last H
                        if ((h_cnt_dat>=2) && (h_cnt_dat<=4)) {// 0.256ms ~ 0.512ms H
                            p_Out8 = 0;
                        } else if ((h_cnt_dat>=7) && (h_cnt_dat<=8)) {// 0.896ms ~ 1.024ms H
                            p_Out8 = 1;
                        } else {
                            l_cnt_dat = 0;
                    ...
                    if (1 == sync_sta) {
                        l_cnt_sync++;
                        if ((l_cnt_sync>=77) && (l_cnt_sync<=79)) {// 9.856ms ~ 10.112ms L
                            sync_sta = 2;// sync stage2 ok
                        } else if (l_cnt_sync > 80) {
                            sync_sta = 0;// too long H
                        }
                    } else {
                        sync_sta = 0;
                        h_cnt_sync = 0;
                ...
                if (f_sync_trig) {
                    ...
                    if ((!f_In12_last) && (l_cnt_dat != 0)) {// Last L
                        if ((l_cnt_dat>=2) && (l_cnt_dat<=4)) {// 0.256ms ~ 0.512ms H
                            p_Out8 = 1;
                        } else if ((l_cnt_dat>=7) && (l_cnt_dat<=8)) {// 0.896ms ~ 1.024ms H
                            p_Out8 = 0;
                        } else {
                            h_cnt_dat = 0;
                            f_sync_trig = 0;// need re-sync
                    ...
                    if ((h_cnt_sync>=2) && (h_cnt_sync<=4)) {// 0.256ms ~ 0.512ms H

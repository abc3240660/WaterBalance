...
//	BIT		f_2k_flag	:	Sys_FlagB.6;	
	...
	BIT		p_press_H		:	Sys_FlagC.3;
	BIT		p_press_V1		:	Sys_FlagC.4;
	BIT		p_press_V2		:	Sys_FlagC.5;
	BIT		p_press_OD		:	Sys_FlagC.6;
	BIT		timeout_str		:	Sys_FlagC.7;

	BYTE	Sys_FlagD	=	0;
	BIT		f_last_V1_on	:	Sys_FlagD.0;
	BIT		f_last_V2_on	:	Sys_FlagD.1;
	BIT		f_last_H1_on	:	Sys_FlagD.2;
	BIT		count_en	    :	Sys_FlagD.3;
	BIT     f_OD_SP_sta     :   Sys_FlagD.4;
	BIT     f_mode_en       :   Sys_FlagD.5;
	BIT		count_load	    :	Sys_FlagD.6;
    BIT     f_sync_trig     :   Sys_FlagD.7;

	BYTE	Sys_FlagE	=	0;
	BIT		p_press_H_last	:	Sys_FlagE.0;
	BIT		p_press_V1_last	:	Sys_FlagE.1;
	BIT		p_press_V2_last	:	Sys_FlagE.2;
	BIT		p_press_OD_last	:	Sys_FlagE.3;
	BIT     p_h_trig     	:   Sys_FlagE.4;
    BIT     p_v1_trig     	:   Sys_FlagE.5;
	BIT     p_v2_trig     	:   Sys_FlagE.6;
	BIT     p_od_trig     	:   Sys_FlagE.7;
			...
			if (1 == start) {
                if (!p_InB_DTA) {// LOW
			        if (f_sync_trig) {
                        ...
                        if (1 == l_low_cnt) {
			    			if ((h_cnt_dat>1) && (h_cnt_dat<6)) {// 0.256ms ~ 0.512ms H
			    				data_sta = 0;
			    				dat_bit_cnt++;
			    			} else if ((h_cnt_dat>5) && (h_cnt_dat<10)) {// 0.896ms ~ 1.024ms H
			    				data_sta = 1;
			    				dat_bit_cnt++;
			    			} else {
			    				dat_bit_cnt = 0;
...
//                        h_cnt_dat = 0;
                        ...
                        h_cnt_dat = 0;
			    		l_low_cnt = 0;
                        if ((h_cnt_sync>1) && (h_cnt_sync<6)) {// 0.256ms ~ 0.512ms H
                    ...
                    if (f_sync_trig) {
                        ...
                        h_cnt_sync++;					
                        if (1 == sync_sta) {
                            if ((l_cnt_sync>74) && (l_cnt_sync<81)) {// 9.856ms ~ 10.112ms L

...
void setup(){
...
#ifdef USE_CERT
    char ssl_error[128];
    while(!InitSSL(comm_ssl_index, pem_CA, pem_cert, pem_pkey, ssl_error)){
        printf("ssl_error = %s\n", ssl_error);
    }
    printf("ssl_error = %s\n", ssl_error);
#else
    char ssl_error[128];
    while(!InitSSL(comm_ssl_index, "", "", "", ssl_error)){
        printf("ssl_error = %s\n", ssl_error);
    }
    printf("ssl_error = %s\n", ssl_error);
#endif
    ...
    while(!SetHTTPEnableSSL(comm_ssl_index)){
        printf("Enable the SSL Fail!\n");
        int e_code;
        if (returnErrorCode(e_code)){
            printf("ERROR CODE: %s\n", e_code);
            printf("Please check the documentation for error details.\n");
